...
void *unified_monitor_thread_func(void *arg) {
    ...
    int old_count = extract_processes(&old_list, &old_sys); 
    ...
    while (1) {
        ...
        //update raw data CSV using snap B
        print_raw_data_to_csv(new_list, new_count);
        write_system_info(new_sys);

        //Calculate individual resources (B vs A)
        int res_count = 0;
        ...
        //update data aggregate CSV using B vs A and cluster
